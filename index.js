...
commonScene.enter(async(ctx) => {
    try{
        ...
        await ctx.reply('What city do you want to live in?', Markup.inlineKeyboard([
...
commonScene.action('sheva_d', async(ctx) => {
    try{
        await ctx.editMessageText('Finally...'),
        ctx.reply('Are you ready?)', Markup.inlineKeyboard([[
            Markup.button.callback('Yes! ', 'yes1')
        ]]))
        global.district = 'Shevchenkivskiy district'
    }catch(e){
        console.error()
    }
})
...
commonScene.action('yes1', (ctx)=>{
    ctx.reply('What budget you have?', Markup.inlineKeyboard([[
...
function addActionBot(name, src, text) {
    bot.action(name, async(ctx)=>{
        try{           
            ...
            if(src !== false){
                await bot.telegram.replyWithPhoto(
                    { source: src },
                    { caption: 'text' }
                )
            }
